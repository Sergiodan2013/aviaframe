...
A lightweight, embeddable flight search widget for travel websites and partner platforms. The canonical production integration uses a public agency widget key and the secure AviaFrame loader.
...
| `data-target-id` | DOM node ID where the widget iframe is mounted | No | `aviaframe-widget` |
...
- `data-agency-key` is public by design and safe to place in page markup.
- The backend validates the website origin against the agency allowlist.
- The browser receives only a short-lived widget session, not supplier credentials.
- DRCT and payment/provider credentials stay server-side.

For the full partner onboarding flow, use [INTEGRATION_GUIDE.md](INTEGRATION_GUIDE.md).
